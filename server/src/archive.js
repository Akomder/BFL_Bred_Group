...
/**
 * Archives the PDF to SharePoint/OneDrive through Microsoft Graph. Forms are
 * filed under <folder>/<YYYY>/<MM>/ so a branch can find a day's slips
 * quickly.
 *
 * Uses the simple upload endpoint, which covers files up to 4 MB — a signed
 * form with one photo sits far below that.
 *
 * Assumes SharePoint is configured — the boot preflight (preflight.js)
 * refuses to start the service otherwise, so there is no local-file
 * fallback here.
 */
export const archiveSubmission = async ({ payload, pdf, fileName }) => {
  ...
  return { archived: true, path: webUrl ?? path }
}
